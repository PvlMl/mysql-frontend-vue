...
<div>
  <div class="d-flex" style="justify-content: space-between">
        ...
        <select class="mb-3" v-model="selectedField">
          <option v-bind:value="id">Дата</option>
          <option v-bind:value="title">Название</option>
          <option v-bind:value="likes">Количество</option>
          <option v-bind:value="hits">Расстояние</option>
        </select>
        <p>По условию:</p>
        <select class="mb-3" v-model="selectedСondition">
          <option v-bind:value="isEqual">Равно</option>
          <option v-bind:value="isIncludes">Содержит</option>
          <option v-bind:value="isMore">Больше</option>
          <option v-bind:value="isLess">Меньше</option>
        </select>
        <input
          type="text"
          class="mb-3"
        ...
        />
        <button class="mb-3 btn btn-primary" @click="goToPrevPage" :disabled="page === 1">Пред.</button>
        <button class="mb-3 ml-3 btn btn-primary" @click="goToNextPage" :disabled="page*10 >= items.length">След.</button>
      </div>
  ...
  data(){
    ...
    methods: {
        ...
        sortByQuantity(){
            ...
            this.paginatedItems = this.items.slice((this.page-1) * 10, this.page * 10);
        },
        sortByDistance(){
        ...
        filteredItems(){
            ...
            return this.paginatedItems
                ...
                if(this.selectedСondition === 'isMore'){
                    ...
                    return this.paginatedItems.filter(
                        el => el[this.selectedField].includes(this.textArea)
                    )
                }
            }
        }
